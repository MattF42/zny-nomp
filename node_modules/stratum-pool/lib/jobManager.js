...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionMask) {
	    ...
	    if( true ) {
	         ...
	         emitLog('Checking blockHex  ' + blockHex + ': which generates a hash of ' + blockHash);
	    } */
       blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
       blockHash = blockHasher(headerBuffer, nTime).toString('hex');
       emitLog('BigInt(job.target) is ' + BigInt(job.target) + ' which needs to be nigger than headerBigNum ' + headerBigNum);
        if (BigInt(job.target) >= headerBigNum) {
            emitLog('BLOCK CANDIDATE FOUND\n');
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, nTime).toString('hex');
        } else {
            ...
            if (shareDiff / difficulty < 0.99) {
                ...
                if (previousDifficulty && shareDiff >= previousDifficulty) {
                    ...
                    // return shareError([23, 'low difficulty share of ' + shareDiff + " vs " + difficulty]);
                    emitLog([23, 'low difficulty share of ' + shareDiff + " vs " + difficulty]);
                }
            }
        }
